...
#' @export WALRUS_step_alt
#' @examples
#' x=1
#' 
WALRUS_step_alt = function(pars, i, t1, t2)
{
  ### FORCING
  ...
  IEO = pars$IT*dt     #Infiltration excess overlandflow threshold 
  if(P_t<IEO)
  {
  PQ      = P_t * i$W                                             *pars$aG
  PV      = P_t * (1-i$W)                                         *pars$aG  
  }else { 
    PQ    = (IEO) * i$W + (P_t - (IEO))                           *pars$aG  
    PV    = (IEO) * i$W                                           *pars$aG 
  }
...
WALRUS_step_alt  = cmpfun(WALRUS_step_alt)
